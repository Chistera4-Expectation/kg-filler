...
class TestItemize(unittest.TestCase):
    ...
    def test_lengthy_elements(self):
        ...
        results = itemize(query_result)
        expected_items = ['Green curry paste', 'Coconut milk', 'Chicken', 'tofu', 'Thai eggplant', 'Bamboo shoots', 'Green beans', 'Kaffir lime leaves', 'Thai basil leaves', 'Fish sauce', 'Palm sugar', 'Green chili peppers']
        for index, result in enumerate(results):
            print(result.value)
            self.assertEqual(result.value, expected_items[index])
    ...
    def test_or_in_element(self):
        ...
        results = itemize(query_result)
        expected_items = ['Whole wheat flour', 'Water', 'Yeast', 'Salt', 'Honey', 'sugar', 'Olive oil', 'butter']
        for index, result in enumerate(results):
            self.assertEqual(result.value, expected_items[index])
